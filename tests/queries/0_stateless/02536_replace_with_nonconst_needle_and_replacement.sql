-- Tests that functions replaceOne() and replaceAll() work with with non-const pattern and replacement arguments
...
INSERT INTO test_tab VALUES (1, 'Hello World', 'l', 'xx') (2, 'Hello World', 'll', 'x') (3, 'Hello World', 'not_found', 'replacement')

SELECT '** replaceAll() **';

SELECT '- non-const needle, const replacement';
SELECT id, haystack, needle, 'x', replaceAll(haystack, needle, 'x')
FROM test_tab
ORDER BY id;

SELECT '- const needle, non-const replacement';
SELECT id, haystack, 'l', replacement, replaceAll(haystack, 'l', replacement)
FROM test_tab
ORDER BY id;

SELECT '- non-const needle, non-const replacement';
SELECT id, haystack, needle, replacement, replaceAll(haystack, needle, replacement)
FROM test_tab
ORDER BY id;
...
SELECT '- non-const needle, const replacement';
SELECT id, haystack, needle, 'x', replaceOne(haystack, needle, 'x')
FROM test_tab
ORDER BY id;


SELECT '- const needle, non-const replacement';
SELECT id, haystack, 'l', replacement, replaceOne(haystack, 'l', replacement)
FROM test_tab
ORDER BY id;


SELECT '- non-const needle, non-const replacement';
SELECT id, haystack, needle, replacement, replaceOne(haystack, needle, replacement)
FROM test_tab
ORDER BY id;
